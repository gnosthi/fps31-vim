 ...
 Based on spf13-vim : Steve Francia's Vim Distributions
...
spf13-vim was a distribution of vim plugins and resources for Vim, Gvim and [MacVim]. However it does not appear to be maintained any longer and some of the bugs were breaking
automated and unattended installs. 
As such I felt the need to fork spf13-vim if for no other reason than to fix some long standing bugs.

spf31-vim and by extension fps31-vim  is a good starting point for anyone intending to use VIM for development running equally well on Windows, Linux, \*nix and Mac.
...
The easiest way to install fps31-vim is to use our [automatic installer](https://j.mp/fps31-vim3) by simply copying and pasting the following line into a terminal. This will install fps31-vim and backup your existing vim configuration. If you are upgrading from a prior version (before 3.0) this is also the recommended installation.

*Requires Git 1.7+ and Vim 7.3+*

```bash

    curl https://j.mp/fps31-vim3 -L > fps31-vim.sh && sh fps31-vim.sh
```

If you have a bash-compatible shell you can run the script directly:
```bash

    sh <(curl https://j.mp/fps31-vim3 -L)
```

## Installing on Windows

On Windows and \*nix [Git] and [Curl] are required. Also, if you haven't done so already, you'll need to install [Vim].
The quickest option to install all three dependencies ([Git], [Curl], [Vim] and [fps31-vim]) is via [Chocolatey] NuGet. After installing [Chocolatey], execute the following commands on the _command prompt_:

    C:\> choco install fps31-vim

_Note: The [fps31-vim package] will install Vim also!_

If you want to install [msysgit], [Curl] and [fps31-vim] individually, follow the directions below.

### Installing dependencies

#### Install [Vim]

After the installation of Vim you must add a new directory to your environment variables path to make it work with the script installation of fps31.
...
#### Installing fps31-vim on Windows

The easiest way is to download and run the fps31-vim-windows-install.cmd file. Remember to run this file in **Administrator Mode** if you want the symlinks to be created successfully.
...
```bash
    ...
    curl https://j.mp/fps31-vim3 -L -o - | sh
...
I'm always happy to take pull requests from others. A good number of people are already [contributors] to [fps31-vim]. Go ahead and fork me.
...
that get loaded *before* the fps31-vim `.vimrc`.

For example, to prevent autocd into a file directory:
```bash
    echo let g:spf13_no_autochdir = 1 >> ~/.vimrc.before.local
```
For a list of available fps31-vim specific customization options, look at the `~/.vimrc.before` file.


### Fork Customization

There is an additional tier of customization available to those who want to maintain a
fork of fps31-vim specialized for a particular group. These users can create `.vimrc.fork`
and `.vimrc.bundles.fork` files in the root of their fork.  The load order for the configuration is:

1. `.vimrc.before` - fps31-vim before configuration
2. `.vimrc.before.fork` - fork before configuration
3. `.vimrc.before.local` - before user configuration
4. `.vimrc.bundles` - fps31-vim bundle configuration
5. `.vimrc.bundles.fork` - fork bundle configuration
6. `.vimrc.bundles.local` - local user bundle configuration
6. `.vimrc` - fps31-vim vim configuration
...
For an example of a fork of fps31-vim that provides customization in this manner see [taxilian's fork](https://github.com/taxilian/spf13-vim).
...
fps31-vim contains a curated set of popular vim plugins, colors, snippets and syntaxes. Great care has been made to ensure that these plugins play well together and have optimal configuration.

## Adding new plugins

Create `~/.vimrc.bundles.local` for any additional bundles.

To add a new bundle, just add one line for each bundle you want to install. The line should start with the word "Bundle" followed by a string of either the vim.org project name or the githubusername/githubprojectname. For example, the github project [fps31/vim-colors](https://github.com/fps31/vim-colors) can be added with the following command

```bash
    echo Bundle \'fps31/vim-colors\' >> ~/.vimrc.bundles.local
```
...
fps31-vim includes the Tagbar plugin. This plugin requires exuberant-ctags and will automatically generate tags for your open files. It also provides a panel to navigate easily via tags

**QuickStart** `CTRL-]` while the cursor is on a keyword (such as a function name) to jump to its definition.

**Customizations**: fps31-vim binds `<Leader>tt` to toggle the tagbar panel
...
fps31-vim ships with a few additional syntaxes:
...
fps31-vim includes [solarized] and [fps31 vim color pack](https://github.com/spf13/vim-colors/):
...
* Certain commands are prefixed with a `<Leader>` key, which by default maps to `\`.
  fps31-vim uses `let mapleader = ","` to change this to `,` which is in a consistent and
  convenient location.
